...
public sealed class FirstLivingEnemyTargetChooser : IEnemyTargetChooser
{
    public string ChooseTargetId(BattleState state, string actorId)
    {
        ...
        BattleCombatantState target = state.Combatants.FirstOrDefault(c => c.Team == targetTeam && c.IsAlive)
